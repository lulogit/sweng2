...
\section{Specific Requirements}
	...
	\begin{requirements}
		 \req { Users must be able to register to the system by providing their
		...
		the system. }
		 \req Required documents include driving license (at least 1 year of experience).
		Payment information consist in credit card 
		 \req { Registered users must be able to find the locations of available
		...
		estimated car autonomy in kilometers.}
		 \req { Among the available cars in a certain geographical region, users
		must be able to reserve a single car for up to one hour before they
		pick it up, by unlocking the car.}
		 \req If a car is not picked\textendash up within one hour from the reservation,
		the system tags the car as available again, and the reservation expires;
		the user pays a fee of 1 EUR. 
		 \req An user is able to cancel the reservation before it's expiration,
		...
		of the current fare through a screen on the car.
		 \req The system debits the user on his card every 10�. Whether the user
