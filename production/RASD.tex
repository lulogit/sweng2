...
\section{Introduction}
	\subsection{Purpose}
		...
		The purpose of our team is to project the system for \powerenjoy, a new born car sharing company. \\
 		The platform will allow clients to find and reserve electric cars using smart devices (described in section 5) and to rent available cars, applying several eco-friendly discounts to incentive user virtuous behaviour.\\
...
\begin{itemize}
	...
	\item{a part of the system (Back-end Process Management) will maximize the efficiency of the services by keeping the fairest possible distribution of charged cars over the territory by: 
		\begin{itemize}
			\item{organize the staff assigning relocation request (also to manage the recharge process of cars)}
			\item{offer discounts to the users that end a rent in a suggested location (Money Saving option). Its amount is computed accordingly to the company's economic return of relocating that car in the suggested destination
			}
		\end{itemize}
	}
...
We have identified useful Actors and situations to model the system to be.
	...
	\begin{domainassumptions}
		...
		\domainassumption{ Each power plug slot has its own park slot }
		\domainassumption{ Users use power plug's park slot only if they charge theirs cars in the power plug associated }
		\domainassumption{ Number of field officiers is sufficient to guarantee the handling of field issues within 15 minutes from the reporting }
	...
	\subsection{Glossary}
		...
		\subsubsection{Rent}
			A \rent is the activity that between a picking up and a release. It can include some \stopover.
		...
		\subsubsection{Staff}
			...
			\begin{itemize}
				...
				\begin{itemize}
				\item They manage the system, for example they can change system parameters. They can add new \safeareas, modify \carsharing prices and create new \staff accounts.
				\end{itemize}
			\end{itemize}
	...
	\subsubsection{Power Plug}
		A \powerplug is the station that charges the battery of an electric car. All \powerplugs are displayed in the map on-board \powerenjoy cars. Each \powerplug is in a \safearea. A \powerenjoyuser who plugs his car into a \powerplug receives the discount expected within \powerenjoy rules.
	\subsubsection{Power Plug Slot}
		A \powerplugslot is a parking that can contain only one car annexed to a \powerplug. Each car parked here is charging.
...
\section{Specific Requirements}
		 ...
		 \req { to rent a car (reserved or available), specifically:
			\begin{itemize}
				...
				\item { to be charged for a given amount of money per minute as soon as the car is unlocked. } 
			 	\item { to be notified of the current fare amount through a screen on the car. }
	...
	\begin{requirementsgroup}
		...
		\req{  to relocate an assigned car:c
			\begin{itemize}
...
\section{Alloy Model}
	...
	\subsection{Models}
		...
		\begin{figure}[H]
				\centering
				\includegraphics[scale=0.3]{./alloy/Scenario1_OK.png}% "%" necessario
				\caption{Emergency}
			\end{figure}
		Here we can see a car accident event. Car2 is a car picked up during a Money Saving Rent (The user has just connected it into a \powerplug). Car Accident1 happens and the car becomes broken. The the emergency staff is alerted. The user pays for his rent.
		\subsubsection{Scenario 2 - Reservation}
			\begin{figure}[H]
				\centering
				\includegraphics[scale=0.8]{./alloy/Scenario2_OK.png}% "%" necessario
				\caption{Reservation}
			\end{figure}
		Here we show a reservation event that occurs. Car1 is available and plugged into a \powerplug. \powerenjoyuser 2 has no reservations or rents active. He reserves Car1 that becomes Car0, an instance of Reservation is created between the \powerenjoyuser (who is become number 1) and  the car. The car changes state and becomes Reserved.

		\subsubsection{Scenario 3 - Payment}
			\begin{figure}[H]
				\centering
				\includegraphics[scale=0.8]{./alloy/Scenario3_OK.png}% "%" necessario
				\caption{Payment}
			\end{figure}
		Here we show an example of payment event. We can see a money saving rent active on the Car2.  In the rent there is a Passenger. The User2 is in a different place from the car because he left it in \stopover connected to a \powerplug. The user decides to end the rent and the car becomes available. A payment is created and the power plug discount is given for this  ride.
