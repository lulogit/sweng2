...
pred evtEndRent[ //p: end rent
	...
	(one d:PassengersDiscount | d in p.discounts) <=> #u.currentRent.passengers >= 2 //f: passengers discount is earned iff the rent's passengers are at least two
	...
	(one d:MoneySavingDiscount | d in p.discounts) <=> (u.currentRent.inputtedDestination=c.currentLocation and u.currentRent in MoneySavingRent)
	(one d:PowerPluggedDiscount | d in p.discounts) <=> ~parkedCars[c] in PowerPlugSlot //f: power plugged discount is earned iff the the car considered is park in a power plug slot
	(one d:BatteryAutonomyDiscount | d in p.discounts) 	<=> c.batteryCharge >= 3 //f: battery autonomy discount is earned iff the battery charge is over the level 3
	(one d:FarFromPowerPlugDiscount | d in p.discounts) <=> (c.batteryCharge <= 1) or (far_from_power_plug[c]) //f: far from power plug discount is charged iff car's battery level is lower than 1 or the car is parked far from power plug
}
